...
function PlanesCarousel() {
    ...
    const getPlanFeatures = (planItem) => {
        const features = [
            ...
            `${planItem.maxPerfil} perfiles simultáneos`
        ];

        if (parseFloat(planItem.precio) > 15) {
            features.push("Contenido 4K UHD", "Descargas ilimitadas", "Sin anuncios");
        } else if (parseFloat(planItem.precio) > 10) {
            features.push("Contenido Full HD", "Descargas limitadas", "Sin anuncios");
        } else {
            features.push("Contenido HD", "Anuncios incluidos");
        }

        return features;
    };

    // ✅ Función para seleccionar plan y mostrar PayPal
    const handleSeleccionarPlan = (planItem) => {
        setPlanSeleccionado(planItem);
        setMostrarPayPal(true);
        console.log("🛒 Plan seleccionado para pago:", planItem);
    };

    // ✅ Cuando el pago es exitoso
    const handlePaymentSuccess = (details) => {
        console.log("✅ Pago exitoso:", details);
        console.log("📦 Plan comprado:", planSeleccionado);
        
        // Aquí puedes:
        // 1. Guardar en tu base de datos
        // 2. Actualizar el estado del usuario
        // 3. Redirigir a otra página
        alert(`¡Pago exitoso! Plan ${planSeleccionado.nombre} activado`);
...
{/* Modal de PayPal - CON SCROLL */}
{mostrarPayPal && planSeleccionado && (
    ...
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl border border-cyan-500/30 shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col">
      ...
      <div className="p-6 border-b border-gray-700/50 flex-shrink-0">
        <div className="text-center">
          ...
          <h3 className="text-white font-bold text-xl mb-2">Confirmar Suscripción</h3>
          ...
          <p className="text-gray-300 mt-2">Plan: <span className="text-cyan-400 font-semibold">{planSeleccionado.nombre}</span></p>
        </div>
      </div>

      {/* Contenido SCROLLABLE */}
      <div className="flex-1 overflow-y-auto p-6">
        {/* Contenedor para PayPal - Altura mínima generosa */}
        <div className="bg-white/5 rounded-xl p-4 border border-gray-600/30 min-h-[400px]">
          <PayPalButton
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-gray-800/40 rounded-lg p-4 border border-gray-700/30">
            <p className="text-cyan-400 text-sm font-semibold mb-2">🎯 Características</p>
            <ul className="text-gray-400 text-sm space-y-1">
              <li>• {planSeleccionado.maxPerfil} perfiles</li>
              <li>• Calidad {planSeleccionado.calidad}p</li>
              <li>• Acceso ilimitado</li>
            </ul>
          </div>
          
          <div className="bg-gray-800/40 rounded-lg p-4 border border-gray-700/30">
            <p className="text-cyan-400 text-sm font-semibold mb-2">🔒 Garantía</p>
            <ul className="text-gray-400 text-sm space-y-1">
              <li>• Cancelación anytime</li>
              <li>• Soporte 24/7</li>
              <li>• Sin contratos</li>
            </ul>
          </div>
        ...
        <div className="mt-6 p-4 bg-gray-800/30 rounded-lg border border-gray-700/20">
          <p className="text-cyan-400 text-sm font-semibold mb-2">🧪 Modo Pruebas Activado</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-400">
            <div>
              ...
              <p className="bg-gray-900/50 p-2 rounded text-xs truncate">
                sb-843pys47569246@personal.example.com
              </p>
            </div>
            <div>
              <span className="font-medium block mb-1">Contraseña:</span>
              <p className="bg-gray-900/50 p-2 rounded text-xs">123456789</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-gray-700/50 flex-shrink-0">
        <button 
          ...
          className="w-full border border-gray-600 text-gray-400 hover:text-white hover:border-gray-400 py-3 rounded-lg transition-all duration-200 flex items-center justify-center gap-2"
        >
          <span>←</span>
          Volver a planes
        </button>
                ...
                <div className="text-center max-w-4xl mx-auto mb-8 sm:mb-12">
                    ...
                    <p className="text-gray-300 text-sm sm:text-base">
                        {plan.length} planes disponibles - Desliza para explorar
                    </p>
                </div>
                            ...
                            return (
                                ...
                                >
                                    <div className={`relative rounded-2xl sm:rounded-3xl p-6 sm:p-8 bg-gradient-to-br ${getPlanColor(index)} backdrop-blur-lg border-2
                                        ${posicion === 'centro'
                                            ? 'border-cyan-400 shadow-2xl shadow-cyan-500/25 w-72 sm:w-88 lg:w-104 h-[420px] sm:h-[480px] lg:h-[580px]'
                                    ...
                                    >
                                        ...
                                        <ul className="space-y-2 mb-6">
                                            {features.slice(0, posicion === 'centro' ? features.length : 3).map((feature, i) => (
                                                <li key={i} className="flex items-start space-x-2 text-white/90">
                                                    <div className={`rounded-full flex items-center justify-center flex-shrink-0 mt-1
                                                         ${posicion === 'centro' 
                                                            ? 'bg-cyan-500/30 w-5 h-5 sm:w-6 sm:h-6' 
                                                            : 'bg-white/20 w-4 h-4 sm:w-5 sm:h-5'
                                                         }`}>
                                                        <span className="text-cyan-300 text-xs sm:text-sm">✓</span>
                                                    </div>
                                                    <span className={`${posicion === 'centro' 
                        ...
                        {plan.map((_, index) => (
                            <button
                                ...
                                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                                    planActivo === index ? 'bg-cyan-400 scale-125' : 'bg-gray-600 hover:bg-gray-400'
